...
class DFlip_ShortCode {
  ...
  public function book( $attr, $content = '', $multi = false ) {
    ...
    if ( empty( $title ) ) {
      $title = $base->get_translate( "text_open_book" );
    }
    ...
    if ( !isset( $attr['thumbtype'] ) ) {
      $html_attr['thumbtype'] = $thumb_tag_type;
    } else {
    ...
    if ( isset( $attr["attachment_pdf_flipbook_lightbox"] ) ) {
      $html_attr["attachment_pdf_flipbook_lightbox"] = $attr["attachment_pdf_flipbook_lightbox"];
    }
    ...
    if ( $type == 'button' || $type == 'thumb' || $type == 'custom' ) {
      $html = '<div class="_df_' . $type . ' ' . esc_attr( $class ) . '" id="' . esc_attr( $id ) . '" ' . $attrHTML . '>' . $title . '</div>';
    } else if ( $type == 'link' ) {
      $html = '<a class="_df_custom ' . esc_attr( $class ) . '" id="' . esc_attr( $id ) . '" href="#" ' . $attrHTML . '>' . $title . '</a>';
    } else {
      $html = '<div class="_df_book df-container df-loading ' . esc_attr( $class ) . '" id="' . esc_attr( $id ) . '" ' . $attrHTML . '></div>';
    }
