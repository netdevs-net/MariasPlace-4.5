...
/**
 * Add Estimate Into Quickbooks Online.
 *
 * @since    1.0.0
	...
	if($this->is_connected()){
		...
		if($this->option_checked('mw_wc_qbo_sync_null_invoice')){
			if($_order_total==0 || $_order_total<0){
				$this->save_log('Export Order Error #'.$ord_id_num,'Order amount 0 not allowed in setting ','Estimate',2);
				return false;
			}
		}
		
		if($this->if_sync_invoice($wc_inv_id,$wc_cus_id,$wc_inv_num)){
			if(!$this->check_quickbooks_estimate_get_obj($wc_inv_id,$wc_inv_num)){
				...
				if($is_send_doc_num){
					$estimate->setDocNumber($DocNumber);
				}				
				...
				*/
				...
				$estimate->setCustomerRef($qbo_customerid);
				...
				if($is_line_item_date){
					$estimate->setTxnDate($qbo_date);
					$estimate->setDueDate($qbo_date);
				}else{
					$estimate->setTxnDate($wc_inv_date);
					$estimate->setDueDate($wc_inv_due_date);
				}
				...
				if($this->is_plugin_active('woocommerce-deposits','woocommmerce-deposits') && $this->option_checked('mw_wc_qbo_sync_enable_wc_deposit')){
					...
					if(is_array($wc_booking_dtls) && count($wc_booking_dtls)){							
						...
						if($_booking_end!=''){
							...
							$estimate->setDueDate($booking_due_date);
						}							
					}						
				}
				...
				*/
				...
				if($qbo_is_sales_tax && is_array($tax_details)){
					...
					if($qbo_is_sales_tax){
						...
						$estimate->setGlobalTaxCalculation($tax_type);
						//$estimate->setApplyTaxAfterDiscount(true);
					}
				...
				if($is_inclusive && !$this->wacs_base_cur_enabled()){
					$estimate->setTotalAmt($this->trim_after_decimal_place($_order_total,7));
				}
				...
				if($this->is_plugin_active('negative-fee-in-deposit-for-myworks-qbo-sync')){
					...
					$estimate->setDeposit($nfd_amount);
					$is_negative_fee_discount_line = false;
					*/
				}
				...
				if(!$_cart_discount && isset($invoice_data['smart_coupons_contribution']) && $this->is_plugin_active('woocommerce-smart-coupons') && $this->option_checked('mw_wc_qbo_sync_compt_wsc_dis_ed') && !empty($this->get_option('mw_wc_qbo_sync_compt_wsc_dis_qbo_item'))){
				...
				/**/
				if(!$qbo_is_shipping_allowed && $is_inclusive && $_order_shipping_tax > 0){
					if($this->wacs_base_cur_enabled()){
						$NetAmountTaxable_Shipping = $order_shipping_total_base_currency;
					}else{
						$NetAmountTaxable_Shipping = $order_shipping_total;
				...
				if($this->is_plugin_active('woocommerce-order-delivery') && $this->option_checked('mw_wc_qbo_sync_compt_p_wod')){
					...
					if($_delivery_date!=''){
						...
						$estimate->setShipDate($_delivery_date);
					}
				}									
				...
				if(!empty($bill_email_addr)){
					...
					$estimate->setBillEmail($BillEmail);
				}				
				...
				if($this->get_array_isset($invoice_data,'_shipping_first_name','',true)!=''){
					...
					$estimate->setShipAddr($ShipAddr);
				}
				
				/*Add  Estimate Note Start*/
				if($customer_note!=''){
					if($this->option_checked('mw_wc_qbo_sync_invoice_notes')){
						...
						if($note_cf_id && $note_cf_name!=''){
							...
							$estimate->addCustomField($Cus_Field);
				...
				if($this->option_checked('mw_wc_qbo_sync_invoice_memo_statement')){
					...
					$estimate->setPrivateNote($first_line_desc);
				}

				/*Add Estimate Note End*/
					...
					if($_wc_shipment_tracking_items!='' || $wf_wc_shipment_source!=''){
						...
						if(!empty($wsti_data)){
							...
							if($tracking_provider!=''){
								$estimate->setShipMethodRef($tracking_provider);
							}
							$estimate->setTrackingNum($tracking_number);
							$estimate->setShipDate($date_shipped);
						}
					}
				}
				...
				if($this->is_plugin_active('woocommerce-hear-about-us') && $this->get_qbo_company_setting('TrackDepartments') && $this->option_checked('mw_wc_qbo_sync_compt_wchau_enable')){
					...
					if($source!=''){
						...
						if($mdp_id){
							$estimate->setDepartmentRef($mdp_id);
							$is_dpt_added = true;
						}
					}
				}
				...
				if(!empty($mw_wc_qbo_sync_inv_sr_txn_qb_department)){
					$estimate->setDepartmentRef($mw_wc_qbo_sync_inv_sr_txn_qb_department);
					$is_dpt_added = true;
				}
				
				$wes_ebay_oth_ord_qb_department = $this->get_wes_eo_qb_loc($invoice_data);
				if(!empty($wes_ebay_oth_ord_qb_department)){
					$estimate->setDepartmentRef($wes_ebay_oth_ord_qb_department);
					$is_dpt_added = true;
				}
				...
				if($this->get_qbo_company_setting('TrackDepartments') && $this->is_plugin_active('myworks-warehouse-routing','mw_warehouse_routing') && $this->option_checked('mw_wc_qbo_sync_mwrqldm_ed')){
					...
					if($wh_loc > 0){
						...
						if($qbloc > 0){
							$estimate->setDepartmentRef($qbloc);
							$is_dpt_added = true;
						}
					}
				}
												...
												if($ivqf){
													...
													if($wcfm_v == 'ShipFromAddr'){
														...
														if(is_array($sfa_a) && !empty($sfa_a)){
															...
															$estimate->setShipFromAddr($ShipFromAddr);
														}
				...
				if($_order_currency!='' && $qbo_home_currency!='' && $_order_currency!=$qbo_home_currency){
					...
					//11-04-2017
					$estimate->setCurrencyRef($_order_currency);
					$estimate->setExchangeRate($currency_rate);
				}

				/*Add Estimate Currency End*/

				$term_id	= (int) $this->get_array_isset($payment_method_map_data,'term_id','',true);
				if($term_id){
					$estimate->setSalesTermRef($term_id);
				}
				...
				if($inv_sr_txn_class!='' && $this->get_qbo_company_setting('ClassTrackingPerTxn')){
					$estimate->setClassRef($inv_sr_txn_class);
				}
				
				/**/
				if($compt_qbo_class_ref!='' && $this->get_qbo_company_setting('ClassTrackingPerTxn')){
					$estimate->setClassRef($compt_qbo_class_ref);
				}
				...
				if($is_automated_sales_tax && !$ast_force_tax_as_li){
					...
					if($TotalTax > 0 || $allow_zero_tax){
						...
						$estimate->addTxnTaxDetail($TxnTaxDetail);
					}												
				}
				...
				if($apply_tax && $is_tax_applied && $Tax_Rate_Ref!=''  && $Tax_Name!=''){
					...
					if($is_qbo_dual_tax && $TaxPercent_2>0){
						...
						$estimate->addSalesTax($SalesTax_2);
					}
					*/
				...
				if($this->get_qbo_company_setting('ETransactionPaymentEnabled')){
					//$estimate->setAllowOnlineCreditCardPayment(true);
					//$estimate->setAllowOnlineACHPayment(true);
				}
				...
				if ($resp = $estimateService->add($Context, $realm, $estimate)){
					$qbo_est_id = $this->qbo_clear_braces($resp);
					$log_title.="Export Order #$ord_id_num\n";
					$log_details.="Order #$ord_id_num has been exported, QuickBooks Estimate ID is #$qbo_est_id";
					$log_status = 1;
					$this->set_imp_sync_data(array('ID'=>$wc_inv_id));
					$this->save_log($log_title,$log_details,'Invoice',$log_status,true,'Add');
					$this->add_qbo_item_obj_into_log_file('Estimate Add',$invoice_data,$estimate,$this->get_IPP()->lastRequest(),$this->get_IPP()->lastResponse(),true);
					...
					if($this->option_checked('mw_wc_qbo_sync_send_inv_sr_afsi_qb')){
						...
						if(!empty($bill_email_addr) || !empty($_billing_email)){
							/*
							if ($resp_send = $estimateService->send($Context, $realm, $qbo_est_id)){
								
							}else{
								$res_send_err = $estimateService->lastError($Context);
							}
							*/
						}
					}
					...
					/**/
					if($this->option_checked('mw_wc_qbo_sync_use_qb_next_ord_num_iowon') && !$this->get_qbo_company_setting('is_custom_txn_num_allowed')){
						$qb_inv_data = $estimateService->query($Context, $realm, "SELECT DocNumber FROM Estimate WHERE Id = '$qbo_est_id'");
						if($qb_inv_data && !empty($qb_inv_data)){
							...
							if(!empty($qbo_inv_doc_no)){
								...
								$_mw_qbo_sync_ord_details['Id'] = $qbo_est_id;
								$_mw_qbo_sync_ord_details['SyncAs'] = 'Estimate';
					...
					$this->save_log($log_title,$log_details,'Invoice',$log_status,true,'Add');
					$this->add_qbo_item_obj_into_log_file('Estimate Add',$invoice_data,$estimate,$this->get_IPP()->lastRequest(),$this->get_IPP()->lastResponse());
