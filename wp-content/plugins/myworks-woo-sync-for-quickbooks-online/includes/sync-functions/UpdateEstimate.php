	...
	if($this->is_connected()){
		...
		if($this->option_checked('mw_wc_qbo_sync_null_invoice')){
			if($_order_total==0 || $_order_total<0){
				$this->save_log('Update Estimate Error #'.$ord_id_num,'Order amount 0 not allowed in setting ','Estimate',2);
				return false;
			}
		}
		
		if($this->if_sync_invoice($wc_inv_id,$wc_cus_id,$wc_inv_num)){
			if($estimate = $this->check_quickbooks_estimate_get_obj($wc_inv_id,$wc_inv_num)){
				//$this->_p($estimate);die;

				//Remove Line Items
				$line_count = $estimate->countLine();
				for($i=0;$i<$line_count;$i++){
					$estimate->unsetLine($i);
				}
				
				//26-04-2017
				for($i=0;$i<=$estimate->countTxnTaxDetail();$i++){
					$estimate->unsetTxnTaxDetail($i);
				}
				...
				if($this->qbo_clear_braces($estimate->getCustomerRef()) != $qbo_customerid){
					$this->save_log('Update Estimate Error #'.$ord_id_num,'Customer Mismatch ','Estimate',0);
					return false;
				}
				...
				*/
				...
				if($this->is_plugin_active('woocommerce-order-delivery') && $this->option_checked('mw_wc_qbo_sync_compt_p_wod')){
					if($estimate->countShipDate()){
						$estimate->unsetShipDate();
					}
				...
				/**/
				if(!$estimate->getDeposit()){
					$estimate->remove('Deposit');
				}
				...
				if ($resp = $estimateService->update($Context, $realm, $estimate->getId(), $estimate)){
					$qbo_inv_id = $this->qbo_clear_braces($estimate->getId());
					$log_title.="Update Order #$ord_id_num\n";
					$log_details.="Order #$ord_id_num has been updated, QuickBooks Estimate ID is #$qbo_inv_id";
					$log_status = 1;
					$this->save_log($log_title,$log_details,'Invoice',$log_status,true,'Update');
					$this->add_qbo_item_obj_into_log_file('Estimate Update',$invoice_data,$estimate,$this->get_IPP()->lastRequest(),$this->get_IPP()->lastResponse(),true);
					...
					if($this->option_checked('mw_wc_qbo_sync_send_inv_sr_afsi_qb')){
						...
						if((!empty($bill_email_addr) || !empty($_billing_email)) && $estimate->getEmailStatus() == 'NeedToSend'){
							if ($resp_send = $estimateService->send($Context, $realm, $qbo_inv_id)){
					...
					$this->save_log($log_title,$log_details,'Invoice',$log_status,true,'Update');
					$this->add_qbo_item_obj_into_log_file('Estimate Update',$invoice_data,$estimate,$this->get_IPP()->lastRequest(),$this->get_IPP()->lastResponse());
