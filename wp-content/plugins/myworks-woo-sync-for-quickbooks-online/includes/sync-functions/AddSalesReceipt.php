			...
			if(!$this->check_quickbooks_salesreceipt_get_obj($wc_inv_id,$wc_inv_num)){
				...
				*/
				...
				if($is_inclusive && !$this->wacs_base_cur_enabled()){
					$SalesReceipt->setTotalAmt($this->trim_after_decimal_place($_order_total,7));
				}				
				...
				$customval1_cfmk = $this->get_cf_cmd_concat_k('customval1',$cfm_iv);
				if($this->is_plugin_active('woocommerce-checkout-field-editor') && isset($cf_map_data[$customval1_cfmk]) && $cf_map_data[$customval1_cfmk]=='CustomerMemo'){
					if(isset($invoice_data['customval1'])){
						$customval1 = $this->get_array_isset($invoice_data,'customval1','',true);
						if(strlen($customval1) > 1000){
							$customval1 = substr($customval1,0,1000);
						}
						$SalesReceipt->setCustomerMemo($customval1);
					}
				}
				...
				if($apply_tax && $is_tax_applied && $Tax_Rate_Ref!=''  && $Tax_Name!=''){
					...
					*/
					...
					/**/
					//$is_dt_nt_li = false;$is_dt_zt_li = false // For Now
					if($is_inclusive && $shipping_method!='' && (($is_dt_nt_li && $is_dt_zt_li) || ($_order_tax > 0 && $_order_shipping_tax <= 0))){
						$zrtc = $this->get_qbo_zero_rated_tax_code($qbo_company_country);
						if(!empty($zrtc) && $qbo_tax_code != $zrtc){
							$Tax_Code_Details_ZR = $this->mod_qbo_get_tx_dtls($zrtc);
							$Tax_Rate_Ref_ZR = (isset($Tax_Code_Details_ZR['TaxRateDetail'][0]['TaxRateRef']))?$Tax_Code_Details_ZR['TaxRateDetail'][0]['TaxRateRef']:'';
							$TaxPercent_ZR = $this->get_qbo_tax_rate_value_by_key($Tax_Rate_Ref_ZR);
							$Tax_Name_ZR = (isset($Tax_Code_Details_ZR['TaxRateDetail'][0]['TaxRateRef']))?$Tax_Code_Details_ZR['TaxRateDetail'][0]['TaxRateRef_name']:'';
							$NetAmountTaxable_ZR = (float) $invoice_data['_order_shipping'];
							//$NetAmountTaxable_ZR = 0;
							
							//
							$TaxLine_ZR = new QuickBooks_IPP_Object_TaxLine();
							$TaxLine_ZR->setAmount(0);
							$TaxLine_ZR->setDetailType('TaxLineDetail');

							$TaxLineDetail_ZR = new QuickBooks_IPP_Object_TaxLineDetail();

							$TaxLineDetail_ZR->setTaxRateRef($Tax_Rate_Ref_ZR);
							$TaxLineDetail_ZR->setPerCentBased('true');
							$TaxLineDetail_ZR->setTaxPercent($TaxPercent_ZR);

							$TaxLineDetail_ZR->setNetAmountTaxable($NetAmountTaxable_ZR);

							$TaxLine_ZR->addTaxLineDetail($TaxLineDetail_ZR);

							$TxnTaxDetail->addTaxLine($TaxLine_ZR);
						}
					}
				...
				if ($resp = $SalesReceiptService->add($Context, $realm, $SalesReceipt)){
					...
					//SR JE
					$enable_payment = (int) $this->get_array_isset($payment_method_map_data,'enable_payment',0);
					if(!empty($payment_data) && $enable_payment && !$enable_batch){
						$enable_transaction = (int) $this->get_array_isset($payment_method_map_data,'enable_transaction',0);
						if($enable_transaction && !$this->option_checked('mw_wc_qbo_sync_sync_txn_fee_as_ng_li')){
							$je_extra_data = array();
							$je_extra_data['wc_inv_no'] = $ord_id_num;
							$je_extra_data['qbo_customer_id'] = $qbo_customerid;
							$je_extra_data['qbo_sr_id'] = $qbo_sr_id;
							
							$je_data = $this->get_je_data_from_pmnt_data($payment_data,$payment_method_map_data,$je_extra_data);
							//$this->_p($je_data);
							if(is_array($je_data) && count($je_data)){
								$this->Sr_AddJournalEntry($je_data);
							}
						}
					}
					
					/*
					$individual_batch_support = (int) $this->get_array_isset($payment_method_map_data,'individual_batch_support',0);
					if($individual_batch_support){
						$this->Cron_Deposit_Sr(array($_payment_method),$wc_inv_id,array($_order_currency));
					}
					*/
					...
					/**/
					if($this->option_checked('mw_wc_qbo_sync_use_qb_next_ord_num_iowon') && !$this->get_qbo_company_setting('is_custom_txn_num_allowed')){
						$qb_sr_data = $SalesReceiptService->query($Context, $realm, "SELECT DocNumber FROM SalesReceipt WHERE Id = '$qbo_sr_id'");
