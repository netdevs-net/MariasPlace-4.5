...
class Wtwp_Admin {
	...
	function wtwp_meta_box_content() {
		...
		if ( 0 < count( $field_data ) ) {
			...
			$html .= '<span class="description">' . __( 'Select testimonial rating.', 'wp-testimonial-with-widget' ) . '</span><strong>'.' '. sprintf( __( 'Utilize these <a href="%s" target="_blank">Premium Features</a> to get best of this plugin with Annual or Lifetime bundle deal..', 'wp-testimonial-with-widget' ), WTWP_PLUGIN_LINK_UNLOCK);
			$html .= '</strong></td><tr/>' . "\n";
	...
	function wtwp_register_menu() {
		...
		// Setting page
		add_submenu_page( 'edit.php?post_type='.WTWP_POST_TYPE, __( 'Solutions & Features - WP Testimonials with rotator widget', 'wp-testimonial-with-widget' ), '<span style="color:#2ECC71">'. __( 'Solutions & Features', 'wp-testimonial-with-widget' ).'</span>', 'manage_options', 'wtwp-solutions-features', array( $this, 'wtwp_solutions_features_page' ) );
	...
	function wtwp_solutions_features_page() {
		include_once( WTWP_DIR . '/includes/admin/settings/solutions-features.php' );
	}
	...
	function testimonial_category_manage_columns( $columns ) {
		...
		$new_columns = array(
							'wtwp_cat_shortcode' => __( 'Testimonial Category Shortcode', 'wp-testimonial-with-widget' )
						);
