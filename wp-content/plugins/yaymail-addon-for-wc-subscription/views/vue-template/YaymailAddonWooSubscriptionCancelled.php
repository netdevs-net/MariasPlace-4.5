<?php
	...
	$subscript_subscription        = false != $order_item_title && isset( $order_item_title['subscript_subscription'] ) ? $order_item_title['subscript_subscription'] : 'Subscription';
	$subscript_price               = false != $order_item_title && isset( $order_item_title['subscript_price'] ) ? $order_item_title['subscript_price'] : 'Price';
	$subscript_last_order_date     = false != $order_item_title && isset( $order_item_title['subscript_last_order_date'] ) ? $order_item_title['subscript_last_order_date'] : 'Last Order Date';
	$subscript_end_of_prepaid_term = false != $order_item_title && isset( $order_item_title['subscript_end_of_prepaid_term'] ) ? $order_item_title['subscript_end_of_prepaid_term'] : 'End of Prepaid Term';
if ( '' === $order ) {
	$subscript_id_value                  = 1;
	$subscript_id_href                   = '#';
	$subscript_price_value               = '£2 / month';
	$subscript_last_order_date_value     = date( 'm-d-Y' );
	$subscript_end_of_prepaid_term_value = date( 'm-d-Y' );
} else {
}
?>
<?php if ( $arrSubscription !== false || '' === $order ) : ?>
<table
	:width="tableWidth"
	cellspacing="0"
	cellpadding="0"
		...
		>
		  ...
		  >
			  <h2 class="yaymail_builder_order" style="font-size: 18px; font-weight: 700;" :style="{color: emailContent.settingRow.titleColor}">
				<?php echo esc_html_e( 'Subscription cancelled', 'woocommerce-subscriptions' ); ?>
			</h2>
				...
				<table class="yaymail_builder_table_items_border yaymail_builder_table_subcription" 
					...
					<thead>
						<tr style="word-break: normal;" :style="{color: emailContent.settingRow.textColor}">
							...
							<th class="td yaymail-title-subscription-end-prepaid-term" scope="col" style="text-align:left;" :style="{'border-color': emailContent.settingRow.borderColor}">{{emailContent.settingRow.titleEndOfPrepaidTerm}}</th>
						</tr>
					</thead>
					...
					<tbody style="flex-direction:inherit;">
						...
						<td class="td"   style="text-align:left; vertical-align:middle;" :style="{'border-color': emailContent.settingRow.borderColor}">
							...
							<td class="td" style="text-align:left; vertical-align:middle;" :style="{'border-color': emailContent.settingRow.borderColor}">
								<?php echo esc_html( date_i18n( wc_date_format(), '' === $order ? $subscript_end_of_prepaid_term_value : $subscription->get_time( 'end', 'site' ) ) ); ?>
							</td>
						</tr>
					</tbody>
				</table>
