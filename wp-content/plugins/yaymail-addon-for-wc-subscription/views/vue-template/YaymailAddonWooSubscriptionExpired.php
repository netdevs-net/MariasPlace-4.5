<?php
	...
	$subscript_end_date        = false != $order_item_title && isset( $order_item_title['subscript_end_date'] ) ? $order_item_title['subscript_end_date'] : 'End date';
if ( '' === $order ) {
	$subscript_id_value              = 1;
	$subscript_id_href               = '#';
	$subscript_price_value           = '£2 / month';
	$subscript_last_order_date_value = date( 'm-d-Y' );
	$subscript_end_date_value        = date( 'm-d-Y' );
} else {
}
?>
		  ...
		  >
			...
			<?php foreach ( $arrSubscription as $key => $subscription ) { ?>
				<table class="yaymail_builder_table_items_border yaymail_builder_table_subcription" 
						...
						style="width: 100% !important;flex-direction:inherit;" width="100%" :style="{'border-color': emailContent.settingRow.borderColor}">
					<thead>
						...
						<th class="td yaymail-title-subscription" scope="col" style="text-align:left;" :style="{'border-color': emailContent.settingRow.borderColor}">{{emailContent.settingRow.titleSubscription}}</th>
							...
							<th class="td yaymail-title-subscription-end-prepaid-term" scope="col" style="text-align:left;" :style="{'border-color': emailContent.settingRow.borderColor}">{{emailContent.settingRow.titleEndDate}}</th>
						</tr>
					</thead>
					...
					<tbody style="flex-direction:inherit;">
						...
						<td class="td"   style="text-align:left; vertical-align:middle;" :style="{'border-color': emailContent.settingRow.borderColor}">
							...
							<td class="td" style="text-align:left; vertical-align:middle;" :style="{'border-color': emailContent.settingRow.borderColor}">
								<?php echo esc_html( date_i18n( wc_date_format(), '' === $order ? $subscript_end_date_value : $subscription->get_time( 'end', 'site' ) ) ); ?>
							</td>
						</tr>
					</tbody>
				</table>
